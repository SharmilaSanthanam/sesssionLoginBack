...
router.post('/register', async (req, res) => {
  ...
  const newUser = new UserSchema({ name, email, password, cpassword })
  bcrypt.hash(password, 7, async (err, hash) => {
    ...
    if (savedUserRes)
      return res.status(200).json({ status: "ok", msg: 'user is successfully saved' })
  })
})
...
router.post(`/login`, async (req, res) => {
  ...
  if (matchPassword) {
    ...
    return res
      .status(200)
      .json({ status: "Ok", msg: 'You have logged in successfully', user, cookie, userSession }) // attach user session id to the response. It will be transfer in the cookies
  } else {
